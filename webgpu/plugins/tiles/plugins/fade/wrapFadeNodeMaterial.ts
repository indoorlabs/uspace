...
const fadeIn = uniform(0).onObjectUpdate(({ material }) =>
  material != null ? ((material as FadeNodeMaterial)?.params?.fadeIn.value ?? 0) : 0,
);
const fadeOut = uniform(0).onObjectUpdate(({ material }) =>
  material != null ? ((material as FadeNodeMaterial)?.params?.fadeOut.value ?? 0) : 0,
);
