import { Mesh, type Texture, MeshBasicNodeMaterial, type NodeMaterial } from 'three/webgpu';
...
function replaceMaterials(mesh: Mesh, overrideMaterial: typeof NodeMaterial): void {
...
export class TileMaterialReplacementPlugin {
  ...
  private readonly handleLoadModel = ({ scene }: TilesRendererEventMap['load-model']): void => {
  ...
  private readonly handleDisposeModel = ({ scene }: TilesRendererEventMap['dispose-model']): void => {
    scene.traverse((object) => {
      if (object instanceof Mesh) {
